...
export async function findWordMatch(text, sourceLang, targetLang) {
  ...
  if (verified[0]) {
    return { translation: verified[0].bm, source: "dictionary", confidence: verified[0].confidence_score, wordId: verified[0].id, pron: verified[0].pron || undefined, label: `Verified — BembaHub dictionary${verified[0].contrib ? " · " + verified[0].contrib : ""}` };
  }
...
export async function updateWordTranslation(en, bm, sourceLang, targetLang) {
  const { rowCount } = await pool.query(
    "UPDATE words SET bm=$1, last_used_at=now(), updated_at=now() WHERE LOWER(TRIM(en))=LOWER(TRIM($2)) AND source_lang=$3 AND target_lang=$4",
    [bm, en, sourceLang || "eng", targetLang || "bem"]
  );
  return rowCount > 0; // lets the caller detect and log a correction that matched nothing, instead of silently reporting success
}
...
export async function addHistory(userId, kind, input, output, source, translationMethod, sourceLang, targetLang) {
  const methodByKind = { translation: "text", camera: "ocr", voice: "voice" };
  const method = translationMethod || methodByKind[kind] || "text";
  const { rows } = await pool.query(
    `INSERT INTO history (user_id,kind,input,output,source,translation_method,source_lang,target_lang) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,
    [userId, kind, input, output, source || "", method, sourceLang || null, targetLang || null]
  );
  return rows[0];
}
...
// Single atomic statement — no separate check-then-act window, so a rapid
// double-tap (very plausible on the mobile-first UI) can never race into the
// UNIQUE(post_id,user_id) constraint and throw an unhandled DB error.
export async function toggleLike(postId, userId) {
  ...
  return rows.length > 0; // true = now liked, false = just unliked
}
...
export async function analyticsSummary() {
  ...
  return {
    ...
    topContributors,
    voice: await voiceAnalytics(),
  };
}

// Real voice-usage stats (spec #21) — every number a live query against
// `history`, nothing invented. Returns empty arrays/zero counts when there's
// genuinely no voice activity yet; the frontend renders "No voice
// translations yet." for that case rather than this function faking a number.
export async function voiceAnalytics() {
  const { rows: totalRow } = await pool.query("SELECT COUNT(*)::int AS n FROM history WHERE kind='voice'");
  const { rows: byPair } = await pool.query(
    `SELECT source_lang, target_lang, COUNT(*)::int AS n FROM history
     WHERE kind='voice' AND source_lang IS NOT NULL AND target_lang IS NOT NULL
     GROUP BY source_lang, target_lang ORDER BY n DESC LIMIT 10`
  );
  const { rows: topSource } = await pool.query(
    `SELECT source_lang, COUNT(*)::int AS n FROM history WHERE kind='voice' AND source_lang IS NOT NULL
     GROUP BY source_lang ORDER BY n DESC LIMIT 5`
  );
  const { rows: topTarget } = await pool.query(
    `SELECT target_lang, COUNT(*)::int AS n FROM history WHERE kind='voice' AND target_lang IS NOT NULL
     GROUP BY target_lang ORDER BY n DESC LIMIT 5`
  );
  return { total: totalRow[0].n, byLanguagePair: byPair, topSourceLangs: topSource, topTargetLangs: topTarget };
}

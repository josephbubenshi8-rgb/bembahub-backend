...
let geminiCooldownReason = null; // "quota" | "overload" — which message to send while active

class QuotaExceededError extends Error {
  constructor(retryAfterSeconds, reason = "quota") {
...
// Distinct from quota exhaustion: Google's own infrastructure temporarily
// overloaded ("high demand"), unrelated to your usage. Confirmed from real
// production logs — repeated identical requests every few seconds during an
// overload window, each one a wasted round trip, since nothing previously
// paused between attempts. Google provides no retryDelay for this case (it's
// not a quota response), so a short fixed cooldown is used instead — long
// enough to stop hammering an already-struggling endpoint, short enough that
// a real user isn't stuck waiting on what's usually a brief spike.
...
async function callGemini(fn, label) {
  ...
  if (now < geminiCooldownUntil) {
    ...
    console.log(`[GEMINI_COOLDOWN_ACTIVE] ${label} — blocked (${geminiCooldownReason}), ${remaining}s remaining, no request sent`);
    throw new QuotaExceededError(remaining, geminiCooldownReason);
  }
  ...
  try {
    ...
    if (isQuotaError(e)) {
      ...
      geminiCooldownReason = "quota";
      console.log(`[GEMINI_QUOTA_EXCEEDED] ${label} — cooldown started, ${secs}s`);
      throw new QuotaExceededError(secs, "quota");
    }
    if (isOverloadError(e)) {
      geminiCooldownUntil = Date.now() + OVERLOAD_COOLDOWN_SECONDS * 1000;
      geminiCooldownReason = "overload";
      console.log(`[GEMINI_OVERLOADED] ${label} — cooldown started, ${OVERLOAD_COOLDOWN_SECONDS}s`);
      throw new QuotaExceededError(OVERLOAD_COOLDOWN_SECONDS, "overload");
    }
    throw e; // some other, unrecognized error — no cooldown, just propagates
  }
}

function sendQuotaResponse(res, err) {
  const message = err.reason === "overload"
    ? "AI translation is temporarily busy due to high demand."
    : "AI translation is temporarily unavailable. Your dictionary translations are still available.";
  res.status(503).json({
    error: err.reason === "overload" ? "AI_OVERLOADED" : "AI_QUOTA_EXCEEDED",
    message,
